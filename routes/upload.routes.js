  ...
  fileFilter(req, file, callback) {
    if (
      ...
      const error = new Error(
        'Поддерживаются JPG, PNG, WebP, AVIF и HEIC',
      );
...
function isManagedWorkImage(imagePath) {
  ...
  const expectedPath =
    `${WORK_UPLOADS_URL}/${fileName}`;

  return (
    value === expectedPath &&
    /^\d{13}-[a-f0-9]{24}\.webp$/.test(
      fileName,
    )
  );
}
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (
        error?.name === 'Error' &&
        /unsupported|corrupt|invalid|Input/i.test(
          error.message,
        )
      ) {
        return res.status(400).json({
    ...
    if (
      ...
      if (
        ...
        return res.status(413).json({
          message:
            'Изображение должно весить не больше 12 МБ',
        });
      }

      return res.status(400).json({
        message:
          'Не удалось принять изображение',
      });
    }
